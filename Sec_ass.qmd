...
\`\`\`{r} hoyde \|\> ggplot() + geom_histogram( data = filter(hoydeNormInc, sex == "male"), mapping = aes(x = age, y = after_stat(density)), alpha = 0.3, colour = "blue", fill = "blue" ) + geom_histogram( data = filter(hoydeNormInc, sex == "female"), mapping = aes(x = age, y = after_stat(density)), alpha = 0.5, colour = "red", fill = "red" ) hoydeNormInc \|\> ggplot( mapping = aes( x = age, y = income ) ) + geom_smooth( formula = y \~ x, method = "lm", colour = "black", lwd = 0.75, se = FALSE )

Histogrammet viser at i gruppen høy inntekt har aldersgruppen hvor det
